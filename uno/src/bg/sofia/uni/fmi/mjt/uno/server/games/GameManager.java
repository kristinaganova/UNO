package bg.sofia.uni.fmi.mjt.uno.server.games;
...
public class GameManager {
    ...
    public boolean joinGame(String gameId, Player player) {
        ...
        notifyPlayersInGame(game, player.getAccount().username() + " has joined the game.");
        return true;
    }
        ...
        return games.values().stream()
                .filter(game -> game.getPlayerRegistry().getPlayers().stream()
                        .anyMatch(player -> player.getAccount() != null &&
                                player.getAccount().username().equals(username)))
                .findFirst()
                .orElse(null);
    }
